...
import { TenantApiKeyGuard } from "../auth/guards/tenant-api-key.guard"
...
export class WebhooksController {
  ...
  @Post(":provider")
  @HttpCode(HttpStatus.OK) // Webhooks typically return 200 OK immediately
  @UseGuards(TenantApiKeyGuard) // Use TenantApiKeyGuard to identify the tenant
  @ApiOperation({ summary: "Receive webhook events from payment providers" })
  @ApiParam({ name: "provider", description: "Name of the payment provider (e.g., paystack, flutterwave, stripe)" })
  @ApiBody({
    schema: {
      type: "object",
  ...
  @ApiResponse({ status: HttpStatus.OK, description: "Webhook received and queued for processing." })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: "Invalid tenant API key or webhook signature." })
  ...
  async receiveWebhook(
    ...
    return this.webhooksService.processIncomingWebhook(tenantId, provider, payload)
  }

  @Post("replay/:id")
  @HttpCode(HttpStatus.OK)
  @UseGuards(JwtAuthGuard) // Replay endpoint should be protected by JWT for internal use
  @ApiOperation({ summary: "Replay a failed or specific webhook event by its ID" })
  @ApiParam({ name: "id", description: "ID of the webhook event to replay" })
  @ApiBearerAuth("access-token")
  @ApiResponse({ status: HttpStatus.OK, description: "Webhook event replayed successfully." })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: "Webhook event not found." })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: "Unauthorized." })
  @ApiResponse({ status: HttpStatus.INTERNAL_SERVER_ERROR, description: "Failed to replay webhook." })
