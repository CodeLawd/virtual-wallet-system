...
export class FlutterwaveService implements IPaymentProvider {
  ...
  async processWebhook(
    ...
    if (
      ...
      payload.event === 'account_number.created' ||
      payload.event === 'account_number.payment'
    ) {
      // This is a mock for a virtual account payment webhook
      this.logger.log(
        `Simulating virtual account payment webhook for Flutterwave.`,
      );
      ...
      this.logger.warn(
        `Unhandled Flutterwave webhook event type: ${payload.event}`,
      );
      return {
        status: 'failed',
